...
func getProxy() string {
	fmt.Printf("Executed")
	proxies, err := utilities.ReadLines("proxy.txt")
	if err == nil {
...
func main() {
	...
	var messages []directmessage.Message
	...
	errrr := json.Unmarshal(bytess, &messages)
	if errrr != nil {
	...
	if option == 1 {
		...
		if mode == 1 {
			...
			for i := 0; i < len(tokens); i++ {
				go func(i int) {
					var message = messages[rand.Intn(len(messages))]
					defer wg.Done()
